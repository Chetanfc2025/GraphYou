...
with open(LABEL_FILE_PATH, "r") as labels_file:
    for line in labels_file:
        ...
        labels.append(int(content[len(features)]))  # Assuming t1 (Emotional Stability) as target
...
try:
    overall_auc = roc_auc_score(y_test, clf.predict_proba(X_test), multi_class="ovr")
except ValueError:
...
class_labels = np.unique(y)  # Dynamically fetch class labels
